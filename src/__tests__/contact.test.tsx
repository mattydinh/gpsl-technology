...
describe("ContactPage", () => {
  ...
  test("defaults to execution topic heading when no query param", () => {
    ...
    render(<ContactPage />);
    expect(
      screen.getByRole("heading", { name: /tell us about the deal or the operation/i, level: 2 })
    ).toBeInTheDocument();
  });
  ...
  test("pre-selects partnerships topic heading when ?topic=partnerships", () => {
    mockGet.mockImplementation((key: string) => (key === "topic" ? "partnerships" : null));
    render(<ContactPage />);
    expect(
      screen.getByRole("heading", { name: /tell us about the partnership/i, level: 2 })
    ).toBeInTheDocument();
  });
  ...
  test("clicking a different topic tile changes the heading and mailto subject", () => {
    ...
    render(<ContactPage />);
    // initially execution
    const technologyTile = screen.getByRole("button", { name: /Technology.*AI agency engagement/i });

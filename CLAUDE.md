...
│   ├── technology/page.tsx # Technology division — #cornerstone (flagship) + #shipped (projects index, redirect target)
...
**Both surfaces theme.** Light/dark toggle flips both — operating swaps warm paper → warm near-black; technology swaps light structural grey → near-black. Accents (terracotta + Monolith orange) each have a light-mode variant tuned for AA contrast on the light bg.
...
**⚠️ `bg-white` is a trap on themed-surface pages.** `bg-white` stays white in dark mode while `text-op-ink` (or `text-tech-ink`) flips to a light color — card titles become invisible. Always use `bg-op-card` / `bg-tech-card` for cards, `bg-op-panel` / `bg-tech-panel` for full-width section lifts. Static zinc/white classes are not allowed on either surface.
...
- Execution is on the `operating` surface; Technology is on the `technology` surface. Both surfaces theme light/dark. Do not cross tokens (`op.*` vs `tech.*`) between them.
...
- **H1s are locked** (see design doc). Home: *"Transformation needs a pathway, not just ambition."* Execution: *"The human layer of GPSL."* Technology: *"Software that ships. Engineers who stay."* Contact: *"Let's talk."*
- **Cornerstone facts on the technology page must stay true of the shipped product** — no invented metrics or certifications.
